...
struct BSMmodel {
  ...
  string trend,                        // type of trend
      ...
      trendOptions = "none/rw/llt/dt", // trend options to select amongst
                                       // (none/rw/llt/td/dt/srw)
      seasonalOptions =
          "none/equal/different", // seasonal options to select amongst
                                  // (none/equal/different/linear)
      irregularOptions =
          "none/arma(0,0)"; // irregular components (none/arma(p,q))
  ...
  vec rhos, // vector indicating whether period is cyclical or seasonal
      ...
      nPar,     // number of parameters in components (trend, cycle, seasonal,
                // irregular)
...
private:
  ...
  int quasiNewtonBSM(std::function<double(vec &, void *)>,
                     std::function<vec(vec &, void *, double, int &)>, vec &,
                     void *, double &, vec &, mat &, bool);
...
public:
  ...
  // Set data
  void setInputs(BSMmodel inputs) { this->inputs = inputs; }
 ...
 ****************************************************/
// Main aux function
void BSMaux(vec, mat, string, int, double, bool, string, vec, bool, bool, vec,
            bool, string, string, string, vec, double, SSinputs &, BSMmodel &);
// Main function
void BSM(vec, mat, string, int, double, bool, string, vec, bool, bool, vec,
         bool, string, string, string, vec, double);
// Convert UC model to PTS
string UC2PTS(string);
// States names for filtering and smoothing
string stateNames(BSMmodel);
// Pre-processing
bool preProcess(vec, mat &, string &, int &, double &, string &, vec, vec,
                int &, string, string, string, vec &, double &);
...
// main aux function
void BSMaux(vec y, mat u, string model, int h, double outlier, bool tTest,
            string criterion, vec periods, bool verbose, bool stepwise, vec p0,
            bool arma, string trendOptions, string seasonalOptions,
            string irregularOptions, vec TVP, double lambda, SSinputs &inputsSS,
            BSMmodel &inputsBSM) {
  ...
  // Pre-processing
  bool errorExit =
      preProcess(y, u, model, h, outlier, criterion, periods, p0, iniObs,
...
// Main function
void BSM(vec y, mat u, string model, int h, double outlier, bool tTest,
         string criterion, vec periods, bool verbose, bool stepwise, vec p0,
         bool arma, string trendOptions, string seasonalOptions,
         string irregularOptions, vec TVP, double lambda) {
  // y:         otuput data (one time series)
  // u:         input data (excluding constant)
  // model:     string with three or four letters with model for error, trend
  // and seasonal h:         forecasting horizon (if inputs it is recalculated
  // as the length differences
  //            between u and y
  // outlier:   standard deviation for outlier detection
  // tTest:     use of unit root tests for stationarity detection true/false
  // criterion: information criterion to use in identification (aic / bic /
  // aicc) periods:   seasonal period verbose:   shows estimation intermediate
  // results stepwise:  use of stepwise faster identification procedure true /
  // false p0:        initial values for parameters to start search arma:
  // testing for arma innovations true / false trendOptions:     set of trends
  // to choose among (none/rw/llt/irw/dt/td) seasonalOptions:  set of seasonal
  // models to choose among (none/equal/different) irregularOptions: set of
  // irregular models to choose amongst (none/arma(0,0)/arma(0,1)/...) TVP:
  // vector of zeros and ones where to allocate TVP parameters for inputs
  ...
  /*
   ...
   BSMmodel inputsBSM;
   // Pre-processing
   bool errorExit = preProcess(y, u, model, h, outlier, criterion, periods, p0,
   iniObs, trendOptions, seasonalOptions, irregularOptions, TVP, lambda); if
   (sum(TVP) > 0) outlier = 0; if (errorExit) Rprintf("%d", errorExit);
   // End of pre-processing
   inputsSS.y = y.rows(iniObs, y.n_elem - 1);
   mat uIni;
   if (iniObs > 0 && u.n_rows > 0){ // && command == "estimate"){
  ...
  SSinputs inputsSS;
  BSMmodel inputsBSM;
  BSMaux(y, u, model, h, outlier, tTest, criterion, periods, verbose, stepwise,
         p0, arma, trendOptions, seasonalOptions, irregularOptions, TVP, lambda,
         inputsSS, inputsBSM);
  // Building model
  ...
  if (inputs.irregular == "?" || inputs.trend == "?" ||
      inputs.seasonal == "?" || inputs.arma)
...
bool preProcess(vec y, mat &u, string &model, int &h, double &outlier,
                ...
                string trendOptions, string seasonalOptions,
                string irregularOptions, vec &TVP, double &lambda) {
  ...
  //     //iniObs = indFinite(min(find(aux3.rows(2, aux3.n_elem - 1) == 3))) +
  //     iMin; iniObs = indFinite(min(find(aux3.rows(nskip - 1, aux3.n_elem - 1)
  //     == nskip))) + iMin;
  // } else {
  //     iniObs = 0;
  // }
  ...
  if (alright == false) {
    Rprintf("%s",
            "ERROR: trendOptions wrongly set (none/rw/irw/llt/dt/td/hd)!!!\n");
    return true;
  }
  ...
  if (alright == false) {
    Rprintf("%s", "ERROR: seasonalOptions wrongly set "
                  "(none/equal/different/linear)!!!\n");
    return true;
  }
  ...
  if (u.n_rows > 0) {
    ...
    if (h < 0) {
      Rprintf("%s",
              "ERROR: Inputs should be at least as long as the ouptut!!!\n");
      return true;
    }
  }
  // Checking periods
  ...
  if (model.find("/") == string::npos) {
    Rprintf("%s", "ERROR: Incorrect number of components "
  ...
  if (comps[1][0] != 'n' && comps[2][0] == 'l') {
    Rprintf("%s", "ERROR: Cycle can estimated only with trigonometric seasonal "
                  "components!!!\n");
    return true;
  }
  if (comps[0][0] == 'n' && comps[1][0] == 'n' && comps[2][0] == 'n' &&
      comps[3][0] == 'n') {
  ...
  if (opt.find(comps[0][0]) == string::npos) {
    Rprintf("%s", "ERROR: Incorrect TREND model (? / none / irw / td / llt / "
                  "dt / srw)!!!\n");
    return true;
  }
  opt = "?n+-0123456789";
  ...
  if (opt.find(comps[2][0]) == string::npos) {
    Rprintf("%s", "ERROR: Incorrect SEASONAL model (? / none / equal / "
                  "different / linear)!!!\n");
    return true;
  }
  if (comps[3][0] == 'a' && model.find("arma") == string::npos) {
    Rprintf("%s",
            "ERROR: Incorrect IRREGULAR model (? / none / arma(p, q))!!!\n");
    return true;
  }
  opt = "?na";
  ...
  //  inputsBSM.compV = {datum::nan};   // Variance of estimated components
  //  (mat) inputsSS.a = {datum::nan};        // Estimated states (mat)
  //  inputsSS.P = {datum::nan};        // Estimated variance of states (mat)
  //  inputsSS.eta = {datum::nan};      // Estimated transition perturbations
  //  (mat) inputsBSM.eps = {datum::nan};     // Estimated observed
  //  perturbations (vec) inputsSS.criteria = {datum::nan}; // Likelihood and
  //  information criteria at optimum (vec) inputsBSM.cycleLimits =
  //  {datum::nan}; inputsBSM.rhos = inputsBSM.periods; inputsBSM.rhos.fill(1);
...
//     //inputs.model = model1;
//     double minCrit;
//     vec y1(inputs.missing.n_elem), y2(inputs.missing.n_elem),
//     aux(SSmodel::inputs.y.n_elem); uvec col(1); col(0) = 1; int iMin =
//     min(inputs.missing), iMax = max(inputs.missing),
...
//         estimUCs(models, inputs.harmonics, minCrit, false, 1e12,
//         SSmodel::inputs.u.n_rows); model = inputs.model;
...
//         aux3 = conv(diff(indFinite), poly);
//         iniObs = indFinite(min(find(aux3.rows(2, aux3.n_elem - 1) == 3))) +
//         iMin; SSmodel::inputs.y = y.rows(iniObs, y.n_elem - 1); if (u.n_rows
//         > 0)
...
//         aux3 = conv(diff(indFinite), poly);
//         finalObs = indFinite(max(find(aux3.rows(0, aux3.n_elem - 3) == 3)) +
//         1); SSmodel::inputs.y = reverse(y.rows(0, finalObs)); if (u.n_rows >
//         0)
...
void BSMclass::setModel(string model, vec periods, vec rhos,
                        bool runFromConstructor) {
...
void BSMclass::bsm2msoe() {
  ...
  // Change R
  SSmodel::inputs.system.R = join_vert(
      SSmodel::inputs.system.R, zeros(nsAdd, SSmodel::inputs.system.R.n_cols));
  // Change T and Z
  ...
  SSmodel::inputs.system.Z =
      join_horiz(SSmodel::inputs.system.Z, zeros(1, nsAdd));
  SSmodel::inputs.system.Z(ns) = 1.0;
  if (seas) {
    if (inputs.seasonal[0] == 'l') {
      SSmodel::inputs.system.T.row(ns + 1) =
          SSmodel::inputs.system.T.row(inputs.ns(0));
      SSmodel::inputs.system.Z
          .cols(0, inputs.ns(0) + inputs.ns(1) + inputs.ns(2) - 1)
          .fill(0.0);
      SSmodel::inputs.system.Z(ns + 1) = 1.0;
    } else {
      SSmodel::inputs.system.Z.cols(0, inputs.ns(0) - 1).fill(0.0);
      SSmodel::inputs.system.T.submat(ns + 1, inputs.ns(0), ns + 1, ns - 1) =
          SSmodel::inputs.system.Z.cols(inputs.ns(0), ns - 1);
    }
  } else {
    SSmodel::inputs.system.Z.cols(0, inputs.ns(0) - 1).fill(0.0);
  }
...
void BSMclass::interpolate(int iniObs) {
  ...
  SSmodel::inputs.y =
      SSmodel::inputs.y.rows(lastObs, SSmodel::inputs.y.n_elem - 1);
  if (SSmodel::inputs.u.n_rows > 0)
    SSmodel::inputs.u =
        SSmodel::inputs.u.cols(lastObs, SSmodel::inputs.u.n_cols - 1);
  estim(false);
  SSmodel::smooth(false);
  ...
  //     SSmodel::inputs.u = reverse(SSmodel::inputs.u, 1);
  // int lastObs = findFirst(SSmodel::inputs.y, sum(inputs.ns));
  // SSmodel::inputs.y = SSmodel::inputs.y.rows(lastObs,
  // SSmodel::inputs.y.n_elem - 1); estim(false); SSmodel::smooth(false); vec
  // yFit = reverse(SSmodel::inputs.yFit); SSmodel::inputs.y =
  // reverse(SSmodel::inputs.y); uvec missing =
  // find_nonfinite(SSmodel::inputs.y.rows(0, iniObs)); inputs = sysCopy;
  // SSmodel::inputs = ssCopy;
...
void BSMclass::estim(bool VERBOSE) {
  ...
  SSmodel::inputs.verbose = VERBOSE;
  if (inputs.trend != "?" && inputs.cycle != "?" && inputs.seasonal != "?" &&
      inputs.irregular != "?") {
    // Particular model
...
void BSMclass::checkModel(uvec harmonics) {
  ...
  if ((ok[10] == 'M' || ok[10] == 'U' || ok[10] == 'O' || ok[10] == 'N') &&
    ...
    setModel(inputs.model, inputs.periods(harmonics), inputs.rhos(harmonics),
             false);
...
void BSMclass::estim(vec p, bool VERBOSE) {
  ...
  flag =
      quasiNewtonBSM(SSmodel::inputs.llikFUN, gradLlik, p, &(SSmodel::inputs),
  ...
  if (SSmodel::inputs.augmented) {
    nTrue =
        nNan2pi - SSmodel::inputs.u.n_rows - SSmodel::inputs.system.T.n_rows;
    uvec stat;
    isStationary(SSmodel::inputs.system.T, stat);
    SSmodel::inputs.nonStationaryTerms =
        SSmodel::inputs.system.T.n_rows - stat.n_elem;
  ...
  infoCriteria(LLIK,
               p.n_elem - SSmodel::inputs.cLlik + SSmodel::inputs.u.n_rows +
                   SSmodel::inputs.nonStationaryTerms,
  ...
  if (flag == 1) {
    ...
    SSmodel::inputs.estimOk =
        "Q-Newton: Maximum Number of iterations reached\n";
  } else if (flag == 5) {
    SSmodel::inputs.estimOk =
        "Q-Newton: Maximum Number of function evaluations\n";
  } else if (flag == 6) {
    SSmodel::inputs.estimOk =
...
void BSMclass::estimUCs(vector<string> allUCModels, uvec harmonics,
                        double &minCrit, bool VERBOSE, double oldMinCrit,
                        int nuInit) {
  ...
  for (unsigned int i = 0; i < allUCModels.size(); i++) {
    ...
    setModel(allUCModels[i], inputs.periods(harmonics), inputs.rhos(harmonics),
             false);
    ...
    // if (SSmodel::inputs.u.n_elem > 0){
    //         if (nuInit > 0){
    //                 SSmodel::inputs.u = SSmodel::inputs.u.rows(0, nuInit -
    //                 1);
    //         } else {
...
void BSMclass::ident(string show, bool VERBOSE) {
  ...
  //     interpolate();
  double season, maxLag, outlierCopy = SSmodel::inputs.outlier;
  string inputTrend = inputs.trend, inputCycle = inputs.cycle,
         inputSeasonal = inputs.seasonal, inputIrregular = inputs.irregular,
         model, trendTypes, cycTypes, seasTypes, irrTypes, restRW;
  int trueTrend, nuInit = SSmodel::inputs.u.n_rows;
  // bool VERBOSE = SSmodel::inputs.verbose;
  ...
  if (inputSeasonal[0] != 'n' && inputSeasonal[0] != 'l' && !inputs.MSOE &&
      periods.n_elem > 1) {
    vec betaHR;
    selectHarmonics(SSmodel::inputs.y, SSmodel::inputs.u, periods, harmonics,
                    betaHR, isSeasonal);
  ...
  if (runAll) { // no stepwise
    ...
    if (inputSeasonal[0] == 'n') {
      ...
      if (inputs.model.substr(0, 1) == "n") {
        ...
        estimUCs(allUCModels, harmonics, minCrit, VERBOSE, minCrit, nuInit);
        // Now take the non trend part of the best model so far and use the DT
        // trend instead
        pos = inputs.model.find("/", 0);
        restRW = inputs.model.substr(pos, inputs.model.size() - pos);
        allUCModels.clear();
  ...
  if (SSmodel::inputs.p.has_nan() || minCrit > 1e10) {
    ...
    setModel("rw/none/none/none", inputs.periods(harmonics),
             inputs.rhos(harmonics), false);
  ...
  if (inputs.arma && succeed) {
    ...
    if (inputIrregular == "?" && succeed) {
      ...
      splitModel(inputs.model, inputTrend, inputCycle, inputSeasonal,
                 inputIrregular2);
      if (inputIrregular == "?" && SSmodel::inputs.y.n_elem > 30) {
        ...
        if ((float)SSmodel::inputs.y.n_elem -
                (float)SSmodel::inputs.system.T.n_rows - 3 - (float)maxLag -
                (float)maxSearch >
            3 * (float)season) {
          filter();
          uvec ind = find_finite(SSmodel::inputs.v);
          // if ((float)SSmodel::inputs.v.n_elem -
          // (float)SSmodel::inputs.system.T.n_rows - 2 > 3 * (float)season){
          if ((float)ind.n_elem - (float)SSmodel::inputs.system.T.n_rows - 2 >
              3 * (float)season) {
            selectARMA(
                SSmodel::inputs.v.rows(SSmodel::inputs.system.T.n_rows + 2,
                                       SSmodel::inputs.v.n_elem - 1),
    ...
    // Selecting best pure ARMA (in case best model is trend + noise of any kind
    // so far)
    pos = inputs.model.find("/none/none/");
    // if (pos < 5 && inputs.model[0] != 'n' && SSmodel::inputs.y.n_elem > 30){
    //     // Searching for pure ARMA when trend + noise has been detected
    //     previously
    //      int maxSearch = season + 4;
    //     if (maxSearch > 28){
    //         maxSearch = 28;
    ...
    //        selectARMA(SSmodel::inputs.y, maxLag, maxSearch, "bic", orders1,
    //        beta1);
    //     }
    ...
    //         allUCModels.push_back(modelNew);
    //         // Estimating potential best model
    //         estimUCs(allUCModels, harmonics, minCrit, VERBOSE, minCrit,
    //         nuInit);
    //     }
    //     if (inputs.model[0] == 'n'){
    //         beta0.reset();
...
void BSMclass::estimOutlier(vec p0, bool VERBOSE) {
  ...
  int n = SSmodel::inputs.y.n_elem - 1, // nNan,
      nu = SSmodel::inputs.u.n_rows, lu = 0;
  // bool VERBOSE = SSmodel::inputs.verbose;
  SSmodel::inputs.verbose = false;
  vec periodsCopy = inputs.periods, rhosCopy = inputs.rhos;
  // Length of u's
  if (nu == 0) {
  ...
  vec valSC;
  if (inputs.ns(0) > 1 &&
      inputs.nPar(0) > 0) { // && SSmodel::inputs.eta.row(1).max() > 3){
  ...
  inputs.typeOutliers = join_vert(
      join_vert(zeros(size(indAO)), ones(size(indLS))), 2 * ones(size(indSC)));
  ...
  if (ind.n_elem > 0) {
    ...
    inputs.rhos = rhosCopy;
    vec obj(1);
    obj(0) = SSmodel::inputs.objFunValue;
    if (obj.is_finite()) {
      // Model with all initial outliers converged
      SSmodel::inputs.verbose = false;
      // Backward deletion step
      uvec remove;
      int ns = SSmodel::inputs.system.T.n_rows, nuAll;
      int count = 0;
      do {
        nuAll = nu + ind.n_elem;
        vec t =
            abs(SSmodel::inputs.betaAug.rows(ns + nu, ns + nuAll - 1) /
...
void BSMclass::components() {
  ...
  if (inputs.ns(3) == 0) { // White noise
    inputs.comp.row(3).cols(0, ny) =
        SSmodel::inputs.y.t() - SSmodel::inputs.yFit.rows(0, ny).t();
    ...
    // inputs.comp.submat(3, 0, 3, SSmodel::inputs.y.n_elem -
    // 1).replace(datum::nan, 0);
    ...
    if (sum(abs(aux)) > 1e-7) {
      inputs.compNames +=
          "ARMA(" + to_string(inputs.ar) + "," + to_string(inputs.ma) + ")/";
    } else
      remove(3) = 1;
  }
  // NaNs in irregular
  ...
  if (k > 0) {
    ...
    if (nOut > 0) {
      ...
      for (int i = 0; i < nOut; i++) {
        ...
        inputs.compNames +=
            namei + to_string((uword)(inputs.typeOutliers(i, 1) + 1)) + "/";
      }
    }
  }
  // inputs.comp.submat(0, 0, inputs.comp.n_rows - 1, SSmodel::inputs.y.n_elem -
  // 1).replace(datum::nan, 0); inputs.comp.submat(0, 0, inputs.comp.n_rows - 1,
  // SSmodel::inputs.y.n_elem - 1).replace(datum::inf, 0);
  if (sum(remove) > 0) {
...
mat BSMclass::parCov(vec &returnP) {
  ...
  uvec isVar = find(inputs.typePar ==
                    0); //, nonConstrained = find(inputs.constPar == 0);
  if (reserveCLLIK) {
    SSmodel::inputs.userModel = bsmMatricesTrue;
    SSmodel::inputs.cLlik = false;
    //    SSmodel::inputs.p(isVar) =log(exp(2 * SSmodel::inputs.p(isVar)) *
    //    SSmodel::inputs.innVariance) / 2;
  }
  returnP = SSmodel::inputs.p;
  mat hess = hessLlik(&(SSmodel::inputs));
  hess *= 0.5 * (nn.n_elem); // - SSmodel::inputs.nonStationaryTerms -
                             // reserveP.n_elem + 1);
...
vec BSMclass::parameterValues(vec p) {
  ...
  if (inputs.nPar(1) > 0) {
    ...
    if (nn > 0) {
      ...
      constrain(pp, inputs.cycleLimits.rows(aux)); // exp(p(0)) / (1+
                                                   // exp(p(0)));
      parValues(arma::span(pos, pos - 1 + nn)) = pp;
      pos = pos + nn;
    }
    // Variances
    // parValues(arma::span(pos, nparCum(1) - 1)) = exp(2 *
    // pCycle(arma::span(nCycles + nn, 2 * nCycles + nn - 1)));
  }
  // ARMA
  if (inputs.ar > 0 || inputs.ma > 0) { // ARMA model
    ...
    if (inputs.ma > 0) {
      ind = regspace<uvec>(nparCum(2) + inputs.ar + 1,
                           nparCum(2) + inputs.ar + inputs.ma);
      polyAux = p(ind);
      polyStationary(polyAux);
      parValues(ind) = polyAux; //(arma::span(1, polyAux.n_elem - 1));
    }
  ...
  //    parValues.rows(nparCum(3), nparCum(3) + nu - 1) =
  //    SSmodel::inputs.betaAug.rows(ind1, ind1 + nu - 1);
  //}
  // pureARMA
  if (inputs.pureARMA) {
...
void BSMclass::parLabels() {
  ...
  if (inputs.seasonal[0] == 'd') {
    char seasNames[20];
    for (unsigned int i = sum(inputs.rhos < 0); i < inputs.periods.n_elem;
         i++) {
...
void BSMclass::validate(bool showTable) {
  ...
  if (nu == 0) {
    ...
    if (SSmodel::inputs.system.Z.n_rows == 1) {
      ...
      stdP = join_vert(
          stdPBSM, sqrt(SSmodel::inputs.betaAugVar.rows(ind1, ind1 + nu - 1)));
    } else {
      uword ind1 = SSmodel::inputs.aEnd.n_rows - nu;
      p = join_vert(scores, SSmodel::inputs.aEnd.rows(
                                ind1, SSmodel::inputs.aEnd.n_rows - 1));
      vec dPEnd = SSmodel::inputs.PEnd.diag();
      stdP = join_vert(stdPBSM,
                       sqrt(dPEnd.rows(ind1, SSmodel::inputs.aEnd.n_rows - 1)));
    }
  }
  vec parValues = p;
  ...
  auto it =
      SSmodel::inputs.table.insert(SSmodel::inputs.table.begin() + 1, str);
  ...
  if (lPer > 0 && periods1(0) > 1 && inputs.nPar(2) > 0) {
    ...
    SSmodel::inputs.table.insert(SSmodel::inputs.table.begin() + 3,
                                 line + "\n");
    insert++;
  } else {
    SSmodel::inputs.table.insert(SSmodel::inputs.table.begin() + 3,
  ...
  if (any(inputs.constPar == 1)) {
    ...
    SSmodel::inputs.table.insert(SSmodel::inputs.table.begin() + 4 + insert,
                                 str);
    insert++;
  }
  if (any(inputs.constPar > 1)) {
  ...
  SSmodel::inputs.table.at(5 + insert) =
    ...
    if (abs(col1(i)) > 1e-3 || abs(col1(i)) == 0 || abs(col1(i)) == 1) {
      if (isnan(pValue(i))) {
        snprintf(str, 70, "%*s: %12.4f%2s \n", 12,
                 inputs.parNames.at(i).c_str(), col1(i), col2.at(i).c_str());
      } else {
  ...
  // if (showTable){
  //     for (auto i = SSmodel::inputs.table.begin(); i !=
  //     SSmodel::inputs.table.end(); i++){
  //         cout << *i << " ";
  //     }
  // }
  if (showTable) {
...
void BSMclass::disturb() {
  ...
  if (inputs.irregular[0] == 'a' && inputs.ar == 0 && inputs.ma == 0) {
    ...
    mat T(nsAll, nsAll),
        Z(1, nsAll); //, R(nsAll, nsAll), Q(nsAll, nsAll); //, D(1, nu);
    ...
    SSmodel::inputs.eta =
        SSmodel::inputs.eta.rows(arma::span(0, inputs.ns(0) - 1));
  }
}
// Gauss-Newton Minimum searcher
// First with numerical gradient function and second with
//                gradient supplied by user
int BSMclass::quasiNewtonBSM(
    std::function<double(vec &x, void *inputsFake)> objFun,
    std::function<vec(vec &x, void *inputsFake, double obj, int &nFuns)>
        gradFun,
    vec &xNew, void *inputsFake, double &objNew, vec &gradNew, mat &iHess,
    bool verbosef) {
  // Code for inputs.constPar
  // 0: not constrained; 1: concentrated-out; 2: zero variance; 3: alpha
  // constrained
  ...
  vec xUncon = xNew;
  // xUncon is xNew de-converted to original variances (xNew is ratio of
  // variances)
  if (cLlik) {
  ...
  do {
    ...
    d_old = d;
    lineSearch(objFun, alpha_i, xNew, objNew, gradNew, d, nIter, nFuns,
               inputsFake);
    ...
    if (cLlik) {
      xUncon(isVar) =
          log(exp(2 * xNew(isVar)) * SSmodel::inputs.innVariance) / 2;
    }
    // Checking for zero variances
    zeroVar = find(
        (((xUncon % (inputs.constPar == 0) % (inputs.typePar == 0)) < -10) +
         ((abs(gradNew) % (inputs.constPar == 0)) % (inputs.typePar == 0) <
          0.0001)) == 2);
    if (zeroVar.n_elem > 0) {
    ...
    if (cLlik) {
      variances =
          exp(2 * xUncon) % (inputs.typePar == 0); //  % (inputs.constPar == 0);
    ...
    // Verbose
    if (verbosef) {
      Rprintf("%5.0i %5.0i %12.5f %12.5f\n", nIter, nOverallFuns, objNew,
              alpha_i);
    }
    // Stop Criteria
    flag = stopCriteria(crit, max(abs(gradNew)), objOld - objNew, 1e5, nIter,
                        nOverallFuns);
...
string stateNames(BSMmodel sys) {
  ...
  if (sys.ns(1) > 0) {
    ...
    for (uword i = 0; i < sys.ns(1); i = i + 2) {
      namesStates += "/Cycle" + to_string(j) + "/Cycle" + to_string(j) + "*";
      j++;
    }
  }
  if (sys.ns(2) > 0) {
...
// Count states and parameters of BSM model
void BSMclass::countStates(vec periods, string trend, string cycle,
                           string seasonal, string irregular) {
  ...
  if (trend[0] == 'l') { // LLT trend
    ...
    inputs.nPar(0) = 2;
             ...
             trend[0] == 's') { // Damped or Hyndman damped trend
    inputs.ns(0) = 2;
    inputs.nPar(0) = 3;
    SSmodel::inputs.exact = false;
    SSmodel::inputs.cLlik = true;
    ...
    inputs.nPar(0) = 1;
             ...
             trend[0] == 't') { // IRW trend or trend with drift
    inputs.ns(0) = 2;
    inputs.nPar(0) = 1;
  } else { // No trend
    inputs.ns(0) = 1;
  ...
  if (trend[0] == 'n' && cycle[0] == 'n' && seasonal[0] == 'n' &&
      irregular[0] == 'a' && SSmodel::inputs.u.n_rows == 0 &&
...
// Fix matrices in standard BSM models (all except variances)
void BSMclass::initMatricesBsm(vec periods, vec rhos, string trend,
                               string cycle, string seasonal,
                               string irregular) {
  ...
  SSmodel::inputs.system.R.submat(0, 0, nsAll - 1, nsCol - 1) =
      eye(nsAll, nsCol);
  /////////****************
  if (nTVP > 0) {
    ...
    SSmodel::inputs.system.R.submat(nsAll - nu, nsCol - 1, nsAll - 1,
                                    nsColTVP - 1) = R;
    // SSmodel::inputs.system.R.print("R final 2823");
    // SSmodel::inputs.system.R.submat(nsAll - nu, nsColTVP - nu, nsAll - 1,
    // nsColTVP - 1) = eye(nu, uniqueTVP.n_elem);
  }
  SSmodel::inputs.system.Q.zeros(nsColTVP, nsColTVP);
  SSmodel::inputs.system.Gam = SSmodel::inputs.system.D =
      SSmodel::inputs.system.S = 0.0;
  SSmodel::inputs.system.Z.zeros(1, nsAll);
  SSmodel::inputs.system.C.ones(1, 1);
  SSmodel::inputs.system.H.zeros(1, 1);
  // Trends
  if (inputs.ns(0) > 0) {
    trend2ss(inputs.ns(0), &SSmodel::inputs.system.T,
             &SSmodel::inputs.system.Z);
  }
  // Cycles
  uvec aux;
  ...
  if (inputs.ns(2) > 0) {
    if (seasonal[0] != 'l') {
      ...
      bsm2ss(inputs.ns(0) + inputs.ns(1), inputs.ns(2), abs(periods(aux)),
             abs(rhos(aux)), &SSmodel::inputs.system.T,
             &SSmodel::inputs.system.Z);
    } else {
      ...
      SSmodel::inputs.system.T(arma::span(i1 + 1, i2), arma::span(i1, i2)) =
          eye(inputs.seas - 2, inputs.seas - 1);
      SSmodel::inputs.system.T(arma::span(i1, i1), arma::span(i1, i2))
          .fill(-1.0);
  ...
  if (sum(inputs.TVP) > 0) {
    SSmodel::inputs.system.Z =
        repmat(SSmodel::inputs.system.Z, SSmodel::inputs.u.n_cols, 1);
    SSmodel::inputs.system.Z.cols(nsAll - SSmodel::inputs.u.n_rows, nsAll - 1) =
        SSmodel::inputs.u.t();
  }
  // Pure ARMA
  if (inputs.pureARMA) {
...
void BSMclass::initParBsm() {
  ...
  if ((SSmodel::inputs.p0(0) == -9999.9) ||
      (indNaN.n_elem == SSmodel::inputs.p0.n_elem)) {
    userP0 = false;
  }
  ...
  } else if (inputs.nPar(0) == 1 &&
             inputs.ns(0) > 1) { // IRW or trend with drift
  ...
  if (inputs.nPar(1) > 0) {
    ...
    inputs.typePar(aux).fill(
        2); // Marking inputs.periods in overall parameter vector
    // Cycle variances
    pos += nRhos;
  ...
  if (inputs.nPar(3) > 1) {
    ...
    // harmonicRegress(SSmodel::inputs.y, SSmodel::inputs.u, periods, betaHR,
    // stdBeta, e); linearARMA(e, orders, inputs.beta0ARMA, stdBeta);
    ...
    if (userP0 && !armas.has_nan()) {
      ...
      if (inputs.ma > 0) {
        ...
        beta0aux =
            inputs.beta0ARMA(arma::span(inputs.ar, inputs.ar + inputs.ma - 1));
  ...
  if (userP0) {
    ...
    if (conc(0) < 1e-6) {
      myError("\n\nERROR: Cannot select such small value for concentrated out "
              "variance!!!\n");
    }
    uvec ind2 = find(inputs.typePar == 0);
    vec variances = p0(ind2) / conc(0);
    if (any(variances < 0)) {
      myError("\n\nERROR: Initial conditions for variances must be "
              "non-negative!!!\n");
    }
    ...
    if (ind.n_elem > 0) {
      ...
      if (any(pp > 1) || any(pp < 0)) {
        myError("\n\nERROR: Initial conditions for damping parameters must be "
                "between 0 and 1!!!\n");
      }
      vec lim1(pp.n_elem);
      lim1.fill(0);
...
void bsmMatrices(vec p, SSmatrix *model, void *userInputs) {
  ...
  if (inp->nPar(2) > 0) { // With seasonal
    ...
    if (inp->seasonal[0] == 'l')
      ...
      model->Q(ind, ind) = eye(inp->ns(2), inp->ns(2)) * exp(2 * p(nparCum(1)));
  ...
  if (inp->nPar(3) == 1) { // Irregular no ARMA model
    ...
    uvec nsCol(1);
    nsCol(0) = nsCum(2); // sum(inp->ns(arma::span(0, 1)));
...
// Variance matrices in standard BSM on top of fixed structure for true
// parameters
void bsmMatricesTrue(vec p, SSmatrix *model, void *userInputs) {
  ...
  if (inp->nPar(2) > 0) { // With seasonal
    ...
    if (inp->seasonal[0] == 'l')
      ...
      model->Q(ind, ind) = eye(inp->ns(2), inp->ns(2)) * p(nparCum(1));
    } else { // Different variances
      ind1 = regspace<uvec>(nparCum(1), 1, nparCum(2) - 1);
...
uvec selectOutliers(vec &val, int nTogether, float limit) {
  ...
  do {
    ...
    if (val(indMax) > limit) {
      ...
      val.rows(std::max(0, (int)indMax - nTogether),
               std::min(n, (int)indMax + nTogether))
          .fill(0);
    } else {
      next = false;
    }
...
// Extract trend seasonal and irregular of model in a string
void splitModel(string model, string &trend, string &cycle, string &seasonal,
                string &irregular) {
  int ind1, ind2, ind3;
  string aux1, aux2;
...
void bsm2ss(int ns0, int nsSeas, vec P, vec rhos, mat *T, mat *Z) {
  ...
  mat aux =
      diagmat(cosines) + diagmat(sines(aux3), 1) + diagmat(-sines(aux3), -1);
  uvec aux2 = regspace<uvec>(0, 1, nDiag + minus);
  (*T)(aux2 + ns0, aux2 + ns0) = aux(aux2, aux2);
}
// Combining models for components
void findUCmodels(string trend, string cycle, string seasonal, string irregular,
    ...
    for (int l = 0; l < nCycleModels; l++) {
      for (int j = 0; j < nSeasonalModels; j++) {
        for (int k = 0; k < nIrregularModels; k++) {
          if (trendModels[i] == "none" && cycleModels[l] == "none" &&
              seasonalModels[j] == "none" && irrModels[k] == "none") {
          } else {
            cModel = trendModels[i];
            cModel.append("/")
                .append(cycleModels[l])
                .append("/")
                .append(seasonalModels[j])
                .append("/")
                .append(irrModels[k]);
...
// Corrects model, cycle string, periods and rhos for modelling cycles
void modelCorrect(string &model, string &cycle, string &cycle0, vec &periods,
                  vec &rhos) {
